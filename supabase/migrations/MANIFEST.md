...
| `0067_agent_runs_recover_expired.sql` | 待应用 | Agent Runner 过期租约恢复 RPC(Cron 扫描器专用):有步骤→interrupted(resumable)/无步骤→failed,原子 UPDATE + generation+1,security definer;只标记不执行 Agent | 
