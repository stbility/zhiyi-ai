 ...
 * 路径数据继承自设计系统 components/icons/Icon.jsx。
 * 唯一的新增是 thumbsUp / thumbsDown —— 设计系统原表里没有,而反馈飞轮
 * 需要这个语义(check/x 读起来是「答案对不对」,不是「有没有用」)。
 * 路径取自 Lucide,与其余图标同源,视觉语言与 24 网格一致。
 *
...
const PATHS = {
  ...
  // 赞 / 踩。设计系统原表里没有这两个,是本项目新增的 ——
  // 反馈飞轮要的正是这个语义,而它在图标集里找不到替代:
  // 先用过 check / x,读起来是「答案对不对」;换成纯文字又太重。
  // 路径取自 Lucide 的 thumbs-up / thumbs-down,与本文件其余图标同源,
  // 所以视觉语言、24 网格、1.75 描边都对得上。
  thumbsUp: (
    <>
      <path d="M7 10v12" />
