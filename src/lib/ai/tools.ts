...
**你只有 3 步。** 每一步是一次完整的模型调用,平均要花一两分钟,
而整次运行必须在平台的时限内结束 —— 所以务必把工具调用合并:
一步之内可以同时请求多个工具,不要一步只做一件小事。
把 list_files / read_file 用在确实需要的时候,不要例行公事地先探一遍。

工作规则:
1. 产出任何代码、配置或文档时,必须用 write_file 写入工作区,不要把文件内容贴在回答正文里。
2. 修改已有文件前,先用 read_file 读一遍确认现状,不要凭记忆改 —— 但可以和别的工具放在同一步里。
3. 工作区是空的、或者任务本来就是从零新建时,不必先 list_files,直接动手。
