...
## 当前状态:Phase 0-3、Phase 4、Phase 7 交付完成,Phase 5/6/8 进行中(2026-08-23)
...
> 环境配置更新(2026-08-23 生产实测):STRIPE_PRICE_* 8 个 Price ID 已配齐
> (status.json stripe_prices_configured=8/8,checkout 走服务端 Checkout 主路径)、
> EMBEDDINGS_API_URL/KEY 已配置(status.json embeddings=true,向量检索链路就绪;
> 模型为 NVIDIA nemotron-3-embed-1b 2048 维,见 docs/env-config-guide.md)、
> RESEND_API_KEY 已配置(status.json email=configured,事务邮件可用)。
...
| 5 | 文件上传、解析、RAG、长期记忆 | 🟡 文件夹上传、跨轮保留、上下文预算已完成;记忆沉淀闭环已实现(0028,确认 → 落库 → Wiki 同步);长期记忆向量召回已上线(0040 pgvector + search_memories,0070 已升级 NVIDIA nemotron-3-embed-1b 2048 维,input_type=passage/query 分流,EMBEDDINGS 生产已配置);AI 记忆管理页已上线(/memory);知识库已上线(0038,解析 + 全文检索 + /knowledge 管理页);向量检索链路已就绪(生产 status.json embeddings=true),向量写入端到端实绩待记忆数据佐证(需真实用户会话沉淀记忆) |
| 6 | Entitlement Service、Stripe 订阅 | 🟡 五档定价(Free/49/149/499/1999)全链路生产运行:checkout/webhook/plans/billing 已上线;STRIPE_PRICE_* 8 个 Price ID 生产已配齐(status.json 8/8,checkout 走服务端 Checkout 主路径);Payment Link 8 个已对齐(2026-08-13 定价 v2:PRO/PRO_PLUS/TEAM/ENT × 月付/年付,见 .env.example,未配时如实 503 降级 Payment Link);权益矩阵 0055 已应用(30 行种子);六项营销承诺全部 gating(2026-08-12 补齐并发任务数 + 历史保留天数);真实付费订阅端到端闭环待首位真实订阅用户验证 |
...
> 上次同步:2026-08-23,main@946446b。本表由 scripts/sync-readme.ts 从 src/lib/phase.ts 生成;
